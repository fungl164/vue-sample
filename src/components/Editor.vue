...
import '../utils/tomorrow.css'
import {bus} from '../utils/bus'

const isMac = CodeMirror.keyMap.default === CodeMirror.keyMap.macDefault

export default {
    props: ['id', 'options', 'mode', 'theme', 'content', 'placeholder', 'editable', 'visible'],
    computed: {
      cmOptions() {
          return {
            // codemirror options
            ...
            json: this.mode === 'text/javascript' || this.mode === 'application/json' ? true : false,
            theme: this.$props.theme || 'tomorrow',
            //   theme: 'base16-dark',
            lineNumbers: true,
            line: true,
            lineWrapping: true,
      ...
      codemirror() {
        return this.$refs[this.id].codemirror
      },
    },
    created() {
      let self = this
      bus.$on('REFRESH_CODEMIRROR', function(id) {
        if (self.id !== id+'-editor') return
        self.codemirror.refresh()
      })
    },
    methods: {
      onCmReady(cm) {
        this.$emit('ready', cm, this.$props.id)
      ...
      onCmFocus(cm) {
        this.$emit('focus', cm, this.$props.id)
      },
      onCmBlur(cm) {
        this.$emit('blur', cm, this.$props.id)
      },
      onCmCodeChange(newCode) {
        this.$emit('input', newCode, this.$props.id)
      }
    },
    components: {
      codemirror
